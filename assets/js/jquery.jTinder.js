...
;(function ($, window, document, undefined) {
	...
	Plugin.prototype = {
		...
		dislike: function() {
			panes.eq(current_pane).animate({"transform": "translate(-" + (pane_width) + "px," + (pane_width*-1.5) + "px) rotate(-10deg)"}, $that.settings.animationSpeed, function () {
		...
		handler: function (ev) {
			...
			switch (ev.type) {
				...
				case 'touchend':
					...
					if (opa >= 1) {
						if (posX > 0) {
							panes.eq(current_pane).animate({"transform": "translate(" + (pane_width) + "px," + (posY + pane_width) + "px) rotate(10deg)"}, $that.settings.animationSpeed, function () {
								if($that.settings.onLike) {
									$that.settings.onLike(panes.eq(current_pane));
								}
								$that.next();
